...
사용자에게는 세 가지 단순한 역할 — **resolver**(오케스트레이터), **coder**(구현자), **reviewer**(읽기 전용 감사자) — 을 보여주며, 자동 승인 권한으로 실행되어 매 단계마다 프롬프트를 띄우지 않고 작업을 완료합니다. Resolver는 계획하고, coder와 reviewer를 디스패치하며, 검증하고, 작업이 완료되거나 명확히 차단될 때까지 반복합니다. 설치 템플릿은 내부용 **explorer**(빠른 읽기 전용 코드베이스 스카우트)와 **deep-reviewer**(철저한 읽기 전용 리뷰) 서브에이전트도 함께 켜서, 사용자가 팀 구성을 직접 하지 않아도 resolver가 빠르게 움직일 수 있게 합니다. 역할을 정의할 뿐 모델 프로바이더를 정의하지 않습니다: 에이전트는 사용자가 지정하지 않는 한 OpenCode 기본 모델을 상속합니다.
...
- **3가지 단순한 사용자용 역할** — `resolver`(오케스트레이터), `coder`(구현자), `reviewer`(읽기 전용 감사자)
- **설치 템플릿의 2가지 내부 서브에이전트** — `explorer`(빠른 코드베이스 스카우트)와 `deep-reviewer`(위험한 변경에 대한 철저한 리뷰)
...
OpenCode를 닫았다가 다시 여세요. 단순한 진입점은 `resolver`, `coder`, `reviewer`이고, resolver는 설치 템플릿의 내부 `explorer`, `deep-reviewer` 서브에이전트도 사용할 수 있습니다.

### 이 템플릿을 사용하는 이유

| 설정 | 이유 |
|---|---|
| `enabled: ["coder", "reviewer", "resolver", "explorer", "deep-reviewer"]` | 5가지 역할 모두 활성화 — 내부 서브에이전트 바로 사용 가능 |
| `autoApprove: true` | Coder와 resolver가 작업별 프롬프트 없이 동작; reviewer/deep-reviewer는 읽기 전용, explorer는 edit-deny 유지 |
| `maxParallelSubagents: 2` | 같은 역할의 서브에이전트가 최대 2개까지 동시 실행 가능 |
...
| `models` 별칭 | 코딩과 탐색에 빠른/저렴한 모델(`quick`/`glm`), 오케스트레이션과 심층 리뷰에 강력한 모델(`gpt`/`deep`) |
...
2. **탐색** (필요시) — 범위가 불명확하면 `explorer`를 디스패치하여 빠른 읽기 전용 코드베이스 탐색을 수행합니다.
3. **구현** — 구성된 역할별 동시성 제한 내에서 `coder`를 디스패치하여 구현합니다.
4. **검증** — 가장 저렴한 의미 있는 검증을 먼저 실행합니다 (대상 테스트, 타입 체크, 린트).
5. **리뷰** — 일반 변경에는 `reviewer` 사용; 위험/보안/아키텍처 변경에는 `deep-reviewer` 사용.
6. **수정** — 문제가 남아있으면, 집중적인 수정과 함께 `coder`를 다시 디스패치합니다.
...
|---|:---:|---|---|---|---|---|
| `resolver` | 예 | `all` | ask → allow | ask → allow | ask → allow | 기본 오케스트레이터. 계획, 구성된 역할별 제한 내에서 coder/reviewer/explorer 디스패치, 검증, 완료까지 반복. |
...
| `explorer` | Postinstall | `subagent` | **deny** | ask → allow | ask → allow | 빠른 읽기 전용 코드베이스 스카우트. 코딩 전 파일, 패턴, API 발견. |
| `deep-reviewer` | Postinstall | `subagent` | **deny** | **deny** | ask → allow | 위험/보안/아키텍처 변경에 대한 철저한 읽기 전용 리뷰. |
...
지원되는 모델 별칭 키: `glm`, `gpt`, `quick`, `deep`, 그리고 지원되는 모든 에이전트 이름. 별칭은 `models`에 정의된 경우에만 해석됩니다.

`preserveNative`은 가독성을 위해 허용되지만, 네이티브 `plan`과 `build`는 항상 보존됩니다. 플러그인은 빌트인 OpenCode 에이전트를 절대 다시 쓰지 않습니다.

### Resolver 오케스트레이션 규칙

Resolver는 속도 우선 접근과 제한된 지속성을 사용합니다:

- 계획 전에 작업을 **quick**, **normal**, **deep**, **risky**로 **분류**.
- 사소한 작업은 직접 검사하고 편집 적용 — 서브에이전트 불필요.
- 범위가 불명확하면 **explorer**를 디스패치하여 빠른 읽기 전용 코드베이스 탐색.
- 집중된 파일/동작 지시와 함께 **coder** 디스패치.
- **가장 저렴한 의미 있는 검증**을 먼저 실행.
- 일반 변경 후 리뷰가 유용할 때 **reviewer** 사용.
- 위험, 보안, 아키텍처, 고영향도 변경에만 **deep-reviewer** 사용.
- 작업이 해결되거나 명확히 차단될 때까지 반복한 후 간결한 요약 반환.
- `maxParallelSubagents` 역할별 제한 준수.
...
- Explorer와 deep-reviewer는 resolver가 속도와 품질을 위해 사용하는 내부 서브에이전트 — 읽기 전용.
- Reviewer와 deep-reviewer는 절대 수정하지 않음 — 수정은 항상 `coder` 또는 `resolver`를 거침.
- Resolver는 `maxParallelSubagents`를 서브에이전트 디스패치의 역할별 동시성 제한으로 준수.
