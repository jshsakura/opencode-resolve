...
const MESSAGES: Record<Locale, Record<MessageKey, MessageTemplate>> = {
  en: {
    ...
    "system.dispatchEscalate": ({ count, agent }) =>
      `🔄 Ralph Loop: ${agent ?? "subagent"} dispatch failed ${count} time(s) in a row. Do NOT retry the same way. Diagnose the ROOT CAUSE first (read the error, use resolve-diagnostics), then re-dispatch with a precise fix.`,
  ...
  ko: {
    ...
    "system.dispatchEscalate": ({ count, agent }) =>
      `🔄 Ralph Loop: 서브에이전트(${agent ?? "subagent"})가 ${count}회 연속 실패했습니다. 같은 방식으로 재시도 금지. 먼저 루트 코즈를 진단하고, 정확한 수정으로 다시 위임하세요.`,
    "system.dispatchStop": ({ count }) =>
