...
export function summarizeTestFailure(stdout: string, stderr: string, runner: string | undefined): string {
  ...
  if (cmd.includes("pytest") || /FAILED\s+\S+::/.test(raw)) {
    ...
    if (failed.length || asserts.length) {
      const parts: string[] = []
      if (failed.length) parts.push(`Failed tests:\n${failed.join("\n")}`)
      if (asserts.length) parts.push(`Assertion errors:\n  ${asserts.join("\n  ")}`)
  ...
  for (const m of out.matchAll(/●\s+(.+)/g)) failingNames.add(m[1].trim().slice(0, 120))     // Jest suite › test
  for (const m of out.matchAll(/^\s*×\s+(.+)/gm)) failingNames.add(m[1].trim().slice(0, 120)) // Vitest × test
  ...
  const parts: string[] = []
  if (failFiles.length) parts.push(`Failed files: ${failFiles.join(", ")}`)
  if (failingNames.size) parts.push(`Failed tests:\n${[...failingNames].slice(0, 10).map((n) => `  ✗ ${n}`).join("\n")}`)
  if (expected.length || received.length) {
    parts.push(`Expected: ${expected[0] ?? "-"}\nReceived: ${received[0] ?? "-"}`)
  }
  if (stack.length) parts.push(`Stack:\n${stack.map((s) => `  at ${s}`).join("\n")}`)
