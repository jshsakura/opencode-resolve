import { SessionState, DIAGNOSTICS_TTL_MS, FAILURE_PATTERN_TTL_MS, FAILURE_THRESHOLD, STRATEGY_PIVOT_THRESHOLD, EDIT_HOTSPOT_TTL_MS, EDIT_HOTSPOT_THRESHOLD, DISPATCH_STOP_THRESHOLD, DISPATCH_PIVOT_THRESHOLD } from "../state.js";
import { classifyBashCommand, detectRollbackCommand, isMissingFileError, formatError, detectProjectContext } from "../utils.js";
...
import { contextMessage, narrate, resolveLocale, t, agentDisplayName, brand, PLUGIN_BRAND, type MessageKey } from "../messages.js";
...
const SELF_REVIEW_NUDGES: ReadonlyArray<readonly [RegExp, string]> = [
...
function hotspotDiffPrompt(filePath: string, count: number): string {
...
const DISPATCH_KEYS: Record<string, MessageKey> = {
...
function dispatchFailed(output: any): boolean {
  if (Boolean(output?.error)) return true
  if (typeof output?.metadata === "object" && output.metadata?.error) return true
  return false
}

export function getHooks(directory: string, options: any, sessionState: SessionState) {
  const narrateToolStart = (input: any, output: any) => {
  ...
  const narrateToolEnd = (input: any, output: any) => {
  ...
  const isActiveResolve = (): boolean => RESOLVE_AGENTS.has(sessionState.currentAgent ?? "")
      ...
      if (evt.type === "lsp.client.diagnostics") {
        ...
        if (props.path) {
          ...
          const errorMessages = diagnostics
            .filter((d: any) => d.severity === 1 || d.severity === "error")
            .slice(0, 20)
            .map((d: any) => ({
        ...
        if (typeof cmd === "string") {
          ...
          if (rollback && await isGitRepository(directory)) {
            ...
            try {
              ...
              throw new Error(
                `[${PLUGIN_BRAND}] refused to run '${cmd}': could not create a rollback checkpoint (${formatError(error)}). ` +
...
"tool.execute.after": async (input: any, output: any) => {
      ...
      const _outText = typeof (output as any)?.output === "string" ? (output as any).output
        : typeof (output as any)?.output?.output === "string" ? (output as any).output.output
        : typeof output === "string" ? output : ""
      if (_outText) sessionState.totalOutputBytes += Buffer.byteLength(_outText, "utf8")
      ...
      if (input.tool === "task") {
        ...
        const dmeta: Record<string, unknown> = { ...(output.metadata ?? {}) }
...
"experimental.session.compacting": async (_input: any, output: any) => {
      ...
      if (contextLines.length > 0) {
        output.context.push(
          `[${PLUGIN_BRAND}] ` + t("compaction.contextHeader", "en", { body: contextLines.join(" ") })
        )
      }
    },
"experimental.chat.messages.transform": async (_input: any, output: any) => {
      ...
      for (const msg of output.messages) {
        for (const part of msg.parts) {
          ...
          const boilerplate = HARNESS_BOILERPLATE_REWRITES.find(([pattern]) =>
            typeof pattern === "string" ? part.text === pattern : pattern.test(part.text))
          if (boilerplate) {
