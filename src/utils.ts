 ...
 */
export function detectRollbackCommand(pattern: string): RollbackKind | undefined {
    const cmd = pattern.trim();
    if (GIT_HARD_RESET_PATTERN.test(cmd)) return "reset"
    if (GIT_FORCE_CLEAN_PATTERN.test(cmd)) return "clean"
    return undefined
}

function isRollbackPermitted(kind: RollbackKind, permissions: ResolvePermissions | undefined): boolean {
    if (kind === "reset") return permissions?.allowGitReset === true
    return permissions?.allowGitClean === true
}

export function classifyBashCommand(pattern: string, permissions?: ResolvePermissions): "allow" | "deny" | "ask" {
    const cmd = pattern.trim();
    ...
    const exempt = new Set<RegExp>();
    if (permissions?.allowGitReset) exempt.add(GIT_HARD_RESET_PATTERN)
    if (permissions?.allowGitClean) exempt.add(GIT_FORCE_CLEAN_PATTERN)

    for (const re of BANNED_COMMANDS) {
    if (!exempt.has(re) && re.test(cmd)) return "deny"
    }
    ...
    if (rollback && isRollbackPermitted(rollback, permissions)) return "allow"
